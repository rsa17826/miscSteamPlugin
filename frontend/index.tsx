...
function runOnDoc(unsafeWindow, doc) {
	// ==UserScript==
	...
	(async () => {
		...
		function savelib(name, obj, liboptions, filename) {
			...
			if (Object.prototype.toString.call(obj) === '[object Object]') {
				...
				if (loadedscripts[name]) {
					if (Object.prototype.toString.call(loadedscripts[name]) === '[object Object]') {
	...
	(() => {
		function newfunc(func, testfunc) {
			...
			var retfunc = function () {
				...
				return testfunc({
					...
					makeenum: function makeenum(thing, _enum) {
						...
						throw new Error(`makeenum: in function ${this.funcname} input ${i} is invalid, should match enum [${_enum}] but is instead ${thing}`);
					}.bind(retfunc),
					...
					ifunset: function ifunset(newinputs) {
						for (var i in newinputs) {
							if (inputargs[i] === undefined) inputargs[i] = newinputs[i];
						}
					}.bind(retfunc),
					/**
					 *
					...
					maketype: function maketype(thing, types) {
						...
						throw new Error(`trymaketype: in function ${this.funcname} input ${i} is invalid, should be type [${types}] but is instead type ${gt(thing)}`);
					}.bind(retfunc),
					/**
					 *
					...
					gettype: function gettype(a, s) {
						...
						throw new Error(`gettype: in function ${this.funcname} input ${i} is invalid, should be type [${s}] but is instead type ${gettype(a)}`);
					}.bind(retfunc),
					/**
			...
			function gt(thing, match) {
				if (
					...
					if (type == 'function') {
						type = /^\s*class\s/.test(Function.prototype.toString.call(thing)) ? 'class' : 'function';
					}
					if (match == 'none') return type == 'nan' || type == 'undefined' || type == 'null';
					try {
		...
		a.randfrom = newfunc(
			function randfrom(min, max) {
				if (max === undefined) return min.length ? min[this(0, min.length - 1)] : this(0, min);
		...
		a.listen = newfunc(
				...
				function listen(elem) {
					...
					if (a.gettype(type, 'object')) {
						...
						a.foreach(type, function (type, cb) {
							if (a.gettype(type, 'string')) type = a.matchall(type, /[a-z]+/g);
							type.forEach((type) => {
			...
			function ({
				...
				if (trygettype(type, ['array', 'any'])) {
					...
					for (var temp of Object.values(type)) gettype(temp, 'function');
				}
			...
			function toelem(elem, single) {
				...
				switch (a.gettype(elem)) {
					...
					case 'object':
						...
						if (single)
							return {
								[Object.keys(newobj)[0]]: a.toelem(newobj[Object.keys(newobj)[0]], single),
							};
		...
		a.geturlperams = newfunc(
			function geturlperams(e = location.href) {
				var arr = {};
				[...e.matchAll(/[?&]([^&\s]+?)(?:=([^&\s]*?))?(?=&|$|\s)/g)].forEach((e) => {
					if (e[1].includes('#')) arr['#'] = e[1].match(/#(.*$)/)[1];
					if (e[2].includes('#')) arr['#'] = e[2].match(/#(.*$)/)[1];
					e[1] = e[1].replace(/#.*$/, '');
					e[2] = e[2].replace(/#.*$/, '');
					arr[decodeURIComponent(e[1]).replaceAll('+', ' ')] = e[2] === undefined ? undefined : decodeURIComponent(e[2]).replaceAll('+', ' ');
				});
				return arr;
			},
		...
		a.updateurlperam = newfunc(
			function updateurlperam(key, value, cangoback) {
				...
				k = k.replace('&', '?');
				k += hash;
				cangoback ? history.pushState(null, null, k) : history.replaceState(null, null, k);
				return key;
			},
			function ({ ifunset, end, maketype, args: [key, value, cangoback] }) {
		...
		a.rerange = newfunc(
			...
			function ({ end, maketype, args: [val, low1, high1, low2, high2] }) {
		...
		a.destring = newfunc(
			function destring(inp) {
				...
				if (a.gettype((out = JSON.parse(inp.replaceAll("'", '"').replaceAll('`', '"'))), 'object')) return out;
		...
		a.remove = newfunc(
			...
			function ({ ifunset, gettype, end, maketype, trymaketype, trygettype, args: [arr, idx, isidx] }) {
				ifunset([undefined, undefined, true]);
		...
		a.createelem = newfunc(
			function createelem(parent, elem, data = {}) {
				...
				elem = issvg ? document.createElementNS('http://www.w3.org/2000/svg', elem) : document.createElement(elem);
				if (data.class)
			...
			function ({ ifunset, end, maketype, args: [parent, elem, data] }) {
		...
		a.newelem = newfunc(
			...
			function ({ ifunset, gettype, end, maketype, trymaketype, trygettype, args: [type, data, inside] }) {
				ifunset([null, {}, []]);
		...
		a.gettype = newfunc(
			...
			function ({ ifunset, gettype, end, maketype, trymaketype, trygettype, args: [_thing, match] }) {
			...
			async function waitforelem(selector) {
				...
				return new Promise((resolve) => {
					...
					function check() {
						...
						observer.disconnect();
						resolve(selector.length == 1 ? a.qs(selector[0]) : selector.map((e) => a.qs(e)));
					}
				});
			},
		...
		a.sha = newfunc(
			function sha(s = '', includesymbols) {
				...
				function str2binb(str) {
					var bin = Array();
					var mask = (1 << 8) - 1;
					for (var i = 0; i < str.length * 8; i += 8) bin[i >> 5] |= (str.charCodeAt(i / 8) & mask) << (24 - i);
					return bin;
				}
				...
				if (delay || waituntildone)
					return new Promise(async (resolve) => {
						if (delay) {
							...
							resolve();
						} else for (var i = 0; i < count; i++) waituntildone ? await func(i) : func(i);
						resolve();
					});
				for (var i = 0; i < count; i++) func(i);
				return;
			},
			function ({ end, args: [func, count, delay, instantstart, waituntildone], maketype }) {
				func = maketype(func, ['function']); // Ensure func is a function
				...
				waituntildone = maketype(waituntildone, ['boolean', 'undefined']); // Ensure waituntildone is a boolean or undefined
				return end();
			},
		);

		a.repeatuntil = newfunc(
			function repeatuntil(func, donecheck, delay, instantstart, waituntildone) {
				return new Promise(async (resolve) => {
					if (delay) {
						var extra = 0;
						var i = 0;
			...
			function ({ end, args: [func, donecheck, delay, instantstart, waituntildone], maketype }) {
				func = maketype(func, ['function']); // Ensure func is a function
		...
		a.getfiles = newfunc(
			...
			function ({ end, args: [oldway, multiple, accept, options], maketype, ifunset }) {
				ifunset(undefined, undefined, [], {});
		...
		a.filteridx = newfunc(
			function filteridx(arr, func) {
				...
				return a.map(arr, (e, i) => (func(e, i) ? i : undefined)).filter((e) => e !== undefined);
			},
			function ({ end, args: [arr, func], maketype }) {
				arr = maketype(arr, ['array', 'object']); // Ensure arr is an array or object
				func = maketype(func, ['function']); // Ensure func is a function
		...
		a.tostring = newfunc(
			function tostring(e) {
				if (['object', 'array'].includes(a.gettype(e))) return JSON.stringify(e);
		...
		a.ispressed = newfunc(
			function ispressed(e /*event*/, code) {
				...
				var temp =
					...
					e.key.toLowerCase() == code.replaceAll(/alt|ctrl|shift|meta/g, '').trim();
		...
		a.controller_vibrate = newfunc(
			function controller_vibrate(pad, duration = 1000, strongMagnitude = 0, weakMagnitude = 0) {
			...
			function ({ end, args: [pad, duration, strongMagnitude, weakMagnitude], maketype }) {
		...
		a.controller_getbutton = newfunc(
			function controller_getbutton(pad, button) {
				return button ? getpad(pad).buttons[button].value : getpad(pad).buttons.map((e) => e.value);
			},
		...
		a.controller_exists = newfunc(
			function controller_exists(pad) {
				return pad === undefined ? getpad().filter((e) => e).length : !!getpad(pad);
			},
		...
		a.indexeddb_setup = newfunc(
							...
							function () {
								...
								var h = function (a) {
									...
									q = function (a, b) {
										...
										for (var c in r) this[c] = 'undefined' != typeof a[c] ? a[c] : r[c];
										...
										this.keyRange = d.IDBKeyRange || d.webkitIDBKeyRange || d.mozIDBKeyRange;
										this.consts = {
											READ_ONLY: 'readonly',
									...
									t = {
										...
										openDB: function () {
											...
											a.onerror = function (a) {
												var b;
												b = 'error' in a.target ? 'VersionError' == a.target.error.name : 'errorCode' in a.target ? 12 == a.target.errorCode : !1;
												if (b) this.onError(Error('The version number provided is lower than the existing one.'));
												else
													(a.target.error
														? (a = a.target.error)
														: ((b = 'IndexedDB unknown error occurred when opening DB ' + this.dbName + ' version ' + this.dbVersion),
															'errorCode' in a.target && (b += ' with error code ' + a.target.errorCode),
															(a = Error(b))),
														this.onError(a));
											}.bind(this);
											...
											a.onupgradeneeded = function (a) {
												this.db = a.target.result(this.db.objectStoreNames.contains(this.storeName))
													? (this.store = a.target.transaction.objectStore(this.storeName))
													: ((a = {
															autoIncrement: this.autoIncrement,
														}),
														null !== this.keyPath && (a.keyPath = this.keyPath),
														(this.store = this.db.createObjectStore(this.storeName, a)));
												var d = Array.prototype.slice.call(this.getIndexList());
												this.indexes.forEach(function (a) {
													var c = a.name;
													c || ((b = !0), this.onError(Error('Cannot create index: No index name given.')));
										...
										put: function (a, b, c, d) {
											...
											var f = !1,
												e = null,
												g = this.db.transaction([this.storeName], this.consts.READ_WRITE);
													...
													(a = g.objectStore(this.storeName).put(
														(() => {
															function isFilesystemHandle(obj) {
																return obj && (obj instanceof FileSystemFileHandle || obj instanceof FileSystemDirectoryHandle);
															}
										...
										batch: function (a, b, c) {
											c || (c = h);
											b || (b = k);
											if ('[object Array]' != Object.prototype.toString.call(a)) c(Error('dataArray argument must be of type Array.'));
											...
											a.forEach(function (a) {
												...
												'remove' == b
													? ((e = g.objectStore(this.storeName)['delete'](d)), (e.onsuccess = l), (e.onerror = a))
													: 'put' == b &&
														(null !== this.keyPath
															? (this._addIdPropertyIfNeeded(e), (e = g.objectStore(this.storeName).put(e)))
										...
										upsertBatch: function (a, b, c, d) {
											...
											c || (c = k);
											b || (b = {});
											'[object Array]' != Object.prototype.toString.call(a) && d(Error('dataArray argument must be of type Array.'));
											var f = b.keyField || this.keyPath,
												e = a.length,
												g = !1,
												l = !1,
												n = 0,
												m = this.db.transaction([this.storeName], this.consts.READ_WRITE);
											...
											a.forEach(function (a) {
												var b = a.key;
												null !== this.keyPath
													? (this._addIdPropertyIfNeeded(a), (a = m.objectStore(this.storeName).put(a)))
										...
										getBatch: function (a, b, c, d) {
											...
											b || (b = k);
											d || (d = 'sparse');
											if ('[object Array]' != Object.prototype.toString.call(a)) c(Error('keyArray argument must be of type Array.'));
											...
											n.onerror = c;
											var m = function (a) {
												a.target.result || 'dense' == d ? f.push(a.target.result) : 'sparse' == d && f.length++;
												e--;
												0 === e && ((g = !0), (l = f));
											};
											a.forEach(function (a) {
										...
										getAll: function (a, b) {
											b || (b = h);
											a || (a = k);
											var c = this.db.transaction([this.storeName], this.consts.READ_ONLY),
												d = c.objectStore(this.storeName);
											d.getAll ? this._getAllNative(c, d, a, b) : this._getAllCursor(c, d, a, b);
											return c;
										},
										...
										indexComplies: function (a, b) {
											return ['keyPath', 'unique', 'multiEntry'].every(function (c) {
												if ('multiEntry' == c && void 0 === a[c] && !1 === b[c]) return !0;
												if ('keyPath' == c && '[object Array]' == Object.prototype.toString.call(b[c])) {
													c = b.keyPath;
													var d = a.keyPath;
													if ('string' == typeof d) return c.toString() == d;
													if (('function' != typeof d.contains && 'function' != typeof d.indexOf) || d.length !== c.length) return !1;
													for (var f = 0, e = c.length; f < e; f++) if (!((d.contains && d.contains(c[f])) || d.indexOf(-1 !== c[f]))) return !1;
													return !0;
												}
												return b[c] == a[c];
											});
			...
			function ({ ifunset, gettype, end, maketype, makeenum, trymaketype, trymakeenum, trygettype, args: [obj] }) {
		...
		a.readfileslow = newfunc(
			function readfileslow(file, type = 'Text', cb1 = (e) => e, cb2 = (e) => e) {
				...
				var readEventHandler = function (evt, idx) {
					if (evt.target.error == null) {
						...
						if (arr.length === lastidx) cb2(arr.sort((e) => e[0]).map((e) => e[1]));
					} else {
		...
		a.randstr = newfunc(
			function randstr({ lower = true, upper = false, number = false, symbol = false, length = 20 }) {
				var rand = '';
				a.repeat(() => {
					rand += a.randfrom(
						`${lower ? 'asdfghjklzxcvbnmqwertyuiop' : ''}${upper ? 'ASDFGHJKLQWERTYUIOPZXCVBNM' : ''}${number ? '0123456789' : ''}${
							symbol ? ',./;\'[]-=\\`~!@#$%^&*()_+|{}:"<>?' : ''
						}`.split(''),
					);
				}, length);
		...
		a.replaceall = newfunc(
			function replaceall(text, regex, replacewith) {
				return text.replaceAll(a.toregex(String(a.toregex(regex)) + 'g'), replacewith);
			},
			function ({ ifunset, gettype, end, maketype, makeenum, trymaketype, trymakeenum, trygettype, args: [text, regex, replacewith] }) {
				text = maketype(text, ['string']);
		...
		a.setrange = newfunc(
			...
			function ({ ifunset, gettype, end, maketype, makeenum, trymaketype, trymakeenum, trygettype, args: [num, min, max] }) {
		...
		a.ondrop = newfunc(
			function ondrop(obj) {
				...
				if (obj.elem && !a.gettype(obj.elem, 'element')) throw new Error(`elem is not an elem, ${oldelem} -> ${obj.elem}`);
				drops.push(obj);
				return obj;
			},
			function ({ ifunset, gettype, end, maketype, makeenum, trymaketype, trymakeenum, trygettype, args: [obj] }) {
		...
		a.step = newfunc(
			...
			function ({ ifunset, gettype, end, maketype, makeenum, trymaketype, trymakeenum, trygettype, args: [num, step] }) {
				num = maketype(num, ['number']);
		...
		a.download = newfunc(
			...
			function ({ ifunset, gettype, end, maketype, makeenum, trymaketype, trymakeenum, trygettype, args: [data, filename, type, isurl] }) {
		...
		a.maketable = newfunc(
			...
			function ({ ifunset, gettype, end, maketype, makeenum, trymaketype, trymakeenum, trygettype, args: [tableData] }) {
	...
	(async () => {
		var i = setInterval(() => {
			if (unsafeWindow.ShowAlertDialog) {
				...
				unsafeWindow.ShowAlertDialog = function (a, s) {
					if (s?.includes?.('added to your account')) {
						return new Promise(() => {});
					}
					// if (a=="View and edit tags for this product"){
					//   ss=o(a,s)
					//   ss.always=function(){}
					//   return ss
					// }
...
webtoon`
		...
		replace.push([/(?<=[^a-z0-9@#$%^&\*x♥]|^)[h%#*♥] ?[e%#*♥] ?[c%#*♥] ?[k%#*♥](?=[^a-z0-9@#$%^&\*x♥]|$)/gi, 'hell']);
